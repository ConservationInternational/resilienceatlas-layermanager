...
## [2.0.2] - 2026-01-08
### Fixed
- Fixed `fetchBounds` in carto-service.js to handle layers that only have `the_geom_webmercator` column instead of `the_geom`
- Uses `COALESCE(the_geom, ST_Transform(the_geom_webmercator, 4326))` to support both geometry column types

## [2.0.1] - 2026-01-06
### Fixed
- Added explicit exports for `./dist/components` and `./dist/layer-manager` paths for backward compatibility
- Added wildcard exports for `./dist/*` and `./lib/*` to allow direct file imports
- Fixed renderLayers logic to prevent duplicate/cancelled layer requests - layers already on the map are no longer re-requested on every render cycle

## [2.0] - 2026-01-06
### Changed
- Updated to Node.js 24 (from 18)
- Updated all Babel packages to latest 7.26.x versions
- Updated Rollup and plugins to latest versions
- Updated dev dependencies (mocha 11.x, chai 5.x, jsdom 26.x, prettier 3.4.x)
- Updated axios to ^1.7.0
- Modernized browserslist config (removed IE support)
- Added package.json exports field for better ESM/CJS support
- Fixed rollup.config.js pkg import with proper JSON import assertion
- Dropped support for React 16 and 17 (now requires React 18+)
